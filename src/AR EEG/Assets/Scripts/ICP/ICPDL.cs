...
public class ICP
{
    public static Tuple<Vector3[], double> TestICP(Vector3[] referencePoints, Vector3[] points) {
        var m = Matrix<double>.Build;
        
        Matrix<double> refMatrix = m.Dense(2, referencePoints.Length);
        Matrix<double> ptMatrix = m.Dense(2, points.Length);

        for (int i = 0; i < referencePoints.Length; i++) {
            refMatrix.SetColumn(i, new double[] { referencePoints[i].x, referencePoints[i].y });
        }

        for (int i = 0; i < points.Length; i++) {
            ptMatrix.SetColumn(i, new double[] { points[i].x, points[i].y });
        }
        ...
        for (int iter = 0; iter < 10; iter++) {
            Tuple<Matrix<double>, Matrix<double>, double> ret = ICP_run(refMatrix, ptMatrix, false);
            R = ret.Item1;
            t = ret.Item2;
            err = ret.Item3;

            //Debug.Log("Iteration " + iter + ": " + err);

            Matrix<double> Third_raw = m.Dense(1, points.Length, 0);
            Matrix<double> Px = m.Dense(1, points.Length);
            Matrix<double> Py = m.Dense(1, points.Length);
            Matrix<double> P_points2;
            P_points2 = R.Multiply(ptMatrix.Stack(Third_raw));
            Px.SetRow(0, P_points2.Row(0));
            Py.SetRow(0, P_points2.Row(1));
            Px = Px + t[0, 0];
            Py = Py + t[1, 0];
            ptMatrix.SetRow(0, Px.Row(0));
            ptMatrix.SetRow(1, Py.Row(0));
        }

        Vector3[] result = new Vector3[points.Length];
        for (int i = 0; i < points.Length; i++) {
            Vector<double> pt = ptMatrix.Column(i);
            result[i] = new Vector3((float)pt[0], (float)pt[1], 0);
        }

        return new Tuple<Vector3[], double>(result, err);
    }
    ...
    internal static Matrix<double> FindCentroid2(Matrix<double> points)
    {
        ...
        centroid[1, 0] = AvrY;
        centroid[2, 0] = AvrZ;
        //Debug.Log(Localization.Time2 + stp.Elapsed.TotalMilliseconds.ToString("0.000"));
        return centroid;
    }
